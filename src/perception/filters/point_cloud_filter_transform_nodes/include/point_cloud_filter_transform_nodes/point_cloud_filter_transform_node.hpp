...
protected:
  ...
  /// \brief subscribe imu message for transform
  ...
  /// \param pt point with x, y, z
  /// \return Transformed point
  PointType transform_point_imu(const PointType & pt) const
  {
    PointType pt_final;
    m_imu_transformer->transform(pt, pt_final);
    return pt_final;
  }
...
private:
  ...
  const typename rclcpp::Subscription<Imu>::SharedPtr m_sub_imu_ptr;
  std::unique_ptr<StaticTransformer> m_imu_transformer;
};
