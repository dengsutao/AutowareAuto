...
geometry_msgs::msg::Pose convert_to_route_pose(
  const lanelet::ConstPoint2d & pt)
{
  geometry_msgs::msg::Pose route_pose;
  route_pose.position.x = pt.x();
  route_pose.position.y = pt.y();
  return route_pose;
}
...
void BehaviorPlanner::set_route(
  const GaodeApiRoute & route)
{

  // create subroutes from given global route
  clear_route();
  // route.segments.front()
  // initialization for before loop
  
  for (size_t i = 0; i < route.enu_route.size(); i++) {
    RouteWithType subroute;
    if (i == 0){
      subroute.route.start_pose = route.start_enu_pose;
    } else {
      subroute.route.start_pose = route.enu_route.at(i); 
    }
    
    if (i == route.enu_route.size() -1){
      subroute.route.goal_pose = route.end_enu_pose;
    } else {
      subroute.route.goal_pose = route.enu_route.at(i); 
    }
    subroute.planner_type = PlannerType::PARKING;
  ...
  //       // set goal to closest poin on lane from starting pose
...
RouteWithType BehaviorPlanner::get_current_subroute(const State & ego_state)
{
  ...
  if (updated_subroute.planner_type == PlannerType::PARKING) {
...
Pose BehaviorPlanner::get_sub_goal()
{
  return m_subroutes.at(m_current_subroute).route.goal_pose;
}
...
void BehaviorPlanner::set_trajectory(const Trajectory & trajectory)
{
  ...
  const auto & route = get_current_subroute().route;
  const auto & last_route_pose = last_point.pose;
  const auto distance = norm_2d(minus_2d(route.goal_pose.position, last_route_pose.position));
