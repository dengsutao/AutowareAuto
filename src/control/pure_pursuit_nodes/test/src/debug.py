...
class ControllerTestingNode2(Node):
    def __init__(self):
        super().__init__("controller_testing_node")
        
        # Publisher
        self._publisher_state = self.create_publisher(
            VehicleKinematicState, "/vehicle/vehicle_kinematic_state", 0
        )
        # self._publisher_odometry = self.create_publisher(
        #     Odometry, 'odom', 10
        # )
        self._publisher_tf = self.create_publisher(
            TFMessage, '/tf_fake', 10
        )
        self._publisher_trajectory = self.create_publisher(
            Trajectory, "/planning/trajectory", 10
        )

    def create_curved_trajectory(self,
            init_state, length, discretization_m,
            speed_start, speed_max, speed_increments, stopping_decel,
            heading_rate, heading_rate_max, heading_rate_increments
        ):
            num_points = int(length / discretization_m)
    ...
    def create_state(self, pose_x, pose_y, pose_ori, v, frame_id) -> VehicleKinematicState:
        state_msg = VehicleKinematicState()
        state_msg.state.pose.position.x = pose_x
        state_msg.state.pose.position.y = pose_y

        state_msg.state.pose.orientation = from_angle(pose_ori)
        state_msg.state.longitudinal_velocity_mps = v
        state_msg.state.lateral_velocity_mps = 0.0  # not modeled in this
        state_msg.state.acceleration_mps2 = 0.0  # modeled as an input
        state_msg.state.heading_rate_rps = 0.0  # modeled as an input
        state_msg.state.front_wheel_angle_rad = 0.0
    ...
    def create_transform(self, now: Time
    ) -> TFMessage:
        transform = TransformStamped()
...
def main():
    ...
    while(1):
        # tf = ctn.create_transform(ctn.get_clock().now())
        # ctn._publisher_tf.publish(tf)
        tf_broadcaster.broadcast_transform()
        ctn._publisher_state.publish(prev_state_msg)
        ctn._publisher_state.publish(current_state_msg)
